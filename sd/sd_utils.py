...
os.environ["CUDA_VISIBLE_DEVICES"] = str(g_store.defaults['general'].gpu)
...
def img2img(prompt: str = '',
    ...
    outpath = g_store.defaults.general.outdir_img2img or g_store.defaults.general.outdir or "outputs/img2img-samples"
    # err = False
    # loopback = False
    # skip_save = False
    ...
    def init():
        ...
        if mask_channel is not None:
            ...
            mask = torch.from_numpy(mask).to(g_store.device)

        if g_store.defaults.general.optimized:
            g_store.modelFS.to(g_store.device)

        init_image = 2. * image - 1.
        init_image = init_image.to(g_store.device)
        init_latent = (g_store.models["model"] if not g_store.defaults.general.optimized else g_store.modelFS).get_first_stage_encoding((g_store.models["model"] if not
        g_store.defaults.general.optimized else g_store.modelFS).encode_first_stage(
    ...
    def sample(init_data, x, conditioning, unconditional_conditioning, sampler_name):
        ...
        if sampler_name != 'DDIM':
            ...
            z_enc = sampler.stochastic_encode(x0,
                                              torch.tensor([t_enc_steps] * batch_size).to(g_store.device))
...
def load_LDSR(checking=False):
    model_name = 'model'
    yaml_name = 'project'
    model_path = os.path.join(g_store.defaults.general.LDSR_dir, 'experiments/pretrained_models',
                              model_name + '.ckpt')
    yaml_path = os.path.join(g_store.defaults.general.LDSR_dir, 'experiments/pretrained_models',
                             yaml_name + '.yaml')
    if not os.path.isfile(model_path):
    ...
    sys.path.append(os.path.abspath(g_store.defaults.general.LDSR_dir))
...
def try_loading_LDSR(model_name: str, checking=False):
    global LDSR
    if os.path.exists(g_store.defaults.general.LDSR_dir):
        try:
...
def load_sd_model(model_name: str) -> [any, any, any, any, any]:
    ckpt_path = g_store.defaults.general.default_model_path
    if model_name != g_store.defaults.general.default_model:
        ckpt_path = os.path.join("models", "custom", f"{model_name}.ckpt")

    if g_store.defaults.general.optimized:
        config = OmegaConf.load(g_store.defaults.general.optimized_config)
        ...
        model.cuda()
        model.eval()
        model.turbo = g_store.defaults.general.optimized_turbo
        ...
        if not g_store.defaults.general.no_half:
            model = model.half()
        ...
        config = OmegaConf.load(g_store.defaults.general.default_model_config)
        model = load_model_from_config(config, ckpt_path)

        device = torch.device(f"cuda:{g_store.defaults.general.gpu}") \
            if torch.cuda.is_available() else torch.device("cpu")
        model = (model if g_store.defaults.general.no_half
...
def generation_callback(img, i=0):
    ...
    if i % int(g_store.defaults.img2img.update_preview_frequency) == 0 and g_store.defaults.img2img.update_preview:
        ...
        if isinstance(img, torch.Tensor):
            x_samples_ddim = (g_store.models["model"] if not g_store.defaults.general.optimized else g_store.modelFS).decode_first_stage(img)
        else:
            # When using the k Diffusion samplers they return a dict instead of a tensor that look like this:
            # {'x': x, 'i': i, 'sigma': sigmas[i], 'sigma_hat': sigmas[i], 'denoised': denoised}
            x_samples_ddim = (g_store.models["model"] if not g_store.defaults.general.optimized else g_store.modelFS).decode_first_stage(img["denoised"])
...
def check_prompt_length(prompt, comments):
    ...
    tokenizer = (g_store.models["model"] if not g_store.defaults.general.optimized else g_store.modelCS).cond_stage_model.tokenizer
    max_length = (g_store.models["model"] if not g_store.defaults.general.optimized else g_store.modelCS).cond_stage_model.max_length

    info = (g_store.models["model"] if not g_store.defaults.general.optimized else g_store.modelCS).cond_stage_model.tokenizer([prompt],
                                                                                                truncation=True,
...
def process_images(outpath,
    ...
    with torch.no_grad(), precision_scope("cuda"), (
        ...
        for n in range(n_iter):
            ...
            uc = (g_store.models["model"] if not g_store.defaults.general.optimized else g_store.modelCS).get_learned_conditioning(
                len(prompts) * [""])
            ...
            if len(weighted_subprompts) > 1:
                ...
                for i in range(0, len(weighted_subprompts)):
                    ...
                    c = torch.add(c, (g_store.models["model"] if not g_store.defaults.general.optimized else g_store.modelCS).get_learned_conditioning(
                        weighted_subprompts[i][0]), alpha=weighted_subprompts[i][1])
            else:  # just behave like usual
                c = (g_store.models["model"] if not g_store.defaults.general.optimized else g_store.modelCS).get_learned_conditioning(prompts)
            ...
            x_samples_ddim = (g_store.models["model"] if not g_store.defaults.general.optimized else g_store.modelFS).decode_first_stage(samples_ddim)
            x_samples_ddim = torch.clamp((x_samples_ddim + 1.0) / 2.0, min=0.0, max=1.0)
            ...
            for i, x_sample in enumerate(x_samples_ddim):
                ...
                if use_RealESRGAN and g_store.models["RealESRGAN"] is not None and use_GFPGAN and g_store.models["GFPGAN"] is not None:
                    # skip_save = True # #287 >_>
                    torch_gc()
                    cropped_faces, restored_faces, restored_img = g_store.models["GFPGAN"].enhance(
                        x_sample[:, :, ::-1], has_aligned=False, only_center_face=False, paste_back=True)
